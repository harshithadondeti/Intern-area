...
function Friends() {
  ...
  return (
    ...
    <span className={`text-purple-800 text-xl p-2  mr-4 ${show ==='requests'?"active":""} `} onClick={setrequests}>Requests </span>
        ...
        <div>
        {
            ...
            UserList.map((user)=>(
                ...
                <div className='flex justify-between userbox bg-slate-50 border-2 rounded-l border-purple-500' >
                    <span>{user.email}</span><span><button className={`bg-purple-500 rounded ${show !== "friends"?'p-1 pr-2 pl-2':''}`}>  { show==="requests"?'Accept': (show==='all-users'?'Make Friend':'')}</button></span>
                </div>
