...
function Friends() {
   ...
   useEffect(()=>{
    const fetchData=async() => {
        try{
            const response=await axios.get("http://localhost:5000/api/User").then((res)=>{
   ...
   useEffect(()=>{
    ...
    const MakeFriend= async(person)=>{
        ...
        await axios.put(`http://localhost:5000/api/User/${person._id}`,{requests:r}).then(res=>{
    ...
    const Accept=async(person)=>{
       const res= await axios.put(`http://localhost:5000/api/User/${person._id}`,{friends:[...person.friends,currentuser.fid]}).then(res=>{
        console.log("test1",res)
        }).catch(err=>{
            console.log(err)
        })

      const res2=  await axios.put(`http://localhost:5000/api/User/${currentuser._id}`,{requests: currentuser.requests.filter(data=>data !== person.fid), friends:[...currentuser.friends,person.fid]}).then(res=>{
