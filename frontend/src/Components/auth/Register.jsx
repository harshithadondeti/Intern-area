...
function Register() {
  ...
  const signup=()=>{
    ...
    setFname(fnameInput.value);
    setLname(lnameInput.value);
...
<span className='border-b w-1/5 lg:w1/4'></span>
              ...
              <div className="mt-4">
                ...
                <input type="text"  value={password} onChange={(e)=>setPassword(e.target.value)} className='text-gray-700 focus:outline-none focus:shadow-outline border border-gray-300 rounded py-2 px-4 block w-full ' id='password' />
              </div> 
              <div className="mt-4 flex justify-between">
<div>
<label htmlFor="Fname" className='border-b text-gray-700 text-sm font-bold mb-2'>First Name</label>
                <input type="text" onChange={(e)=>setFname(e.target.value)} className='text-gray-700 focus:outline-none focus:shadow-outline border border-gray-300 rounded py-2 px-4 block w-full '
                 id='Fname' value={fname} />
</div>
<div className='ml-5'>
<label htmlFor="Lname" className='border-b text-gray-700 text-sm font-bold mb-2'>Last Name</label>
                <input type="text" onChange={(e)=>setLname(e.target.value)} className='text-gray-700 focus:outline-none focus:shadow-outline border border-gray-300 rounded py-2 px-4 block w-full appearance-none'
                 id='Lname'  value={lname} />
</div>
              </div>
              <small>By signing up, you agree to our <span className='text-blue-400'>Term and Conditions.
                </span></small>
                <button className='bg-blue-500 h-9 text-white font-bold py-2 mt-4 px-4 w-full rounded hover:bg-blue-600' onClick={signup} >Sing Up </button>
                Already registered? <Link to={'/login'}> <span className='text-blue-400 cursor-pointer' >Login</span> </Link>
</div>
          </div>
        </div>
      </div>
